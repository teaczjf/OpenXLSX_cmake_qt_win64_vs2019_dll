 ...
 */
  ...
  DWORD ptw32_relmillisecs (const struct timespec_pthread * abstime);
  ...
  void ptw32_timespec_to_filetime (const struct timespec_pthread *ts, FILETIME * ft);
  void ptw32_filetime_to_timespec (const FILETIME * ft, struct timespec_pthread *ts);
#endif
